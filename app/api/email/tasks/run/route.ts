...
async function refreshCampaignCounts(admin: ReturnType<typeof getSupabaseAdmin>, campaignId: string) {
  ...
  await admin
    ...
    .update({
      ...
      next_run_at: done ? null : new Date(Date.now() + 15 * 60 * 1000).toISOString(),
      sent_count: sent,
      failed_count: failed,
      opened_count: opened,
    ...
    .eq('id', campaignId);
  return { sent, failed, queued, opened, clicked };
}
...
async function runTasks(request: Request) {
  try {
    ...
    for (const campaign of (campaigns || []) as Array<Record<string, unknown>>) {
      const profile = ((campaign.email_sending_profiles as EmailSenderProfile | null) || senderProfileFromEnv({
      ...
      if (profile.is_enabled === false) {
        ...
        results.push({ campaignId: campaign.id, sent: 0, failed: 0, paused: true });
        continue;
      }

      await admin
        .from('email_campaigns')
        .update({ status: 'sending', started_at: campaign.started_at || now, last_run_at: now })
        .eq('id', campaign.id);
