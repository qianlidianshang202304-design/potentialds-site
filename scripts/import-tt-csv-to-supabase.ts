...
async function main() {
  ...
  for await (const record of parser) {
    ...
    if (buffer.length >= batchSize) {
      await flush(buffer, influencers, dryRun);
      inserted += buffer.length;
      buffer.length = 0;
      process.stdout.write(`Processed ${processed}, Inserted ${inserted} rows...\n`);
      await sleep(1000);
    }
  }
  ...
  process.stdout.write(`Done. processed=${processed}, inserted=${inserted}${dryRun ? ' (dry-run)' : ''}\n`);
}

async function flush(buffer: InfluencerInsert[], influencers: any, dryRun: boolean) {
  const knownColumns = ['platform', 'nickname', 'username', 'fans_num', 'view_avg', 'region', 'region_zh', 'tags', 'link', 'email', 'uid', 'products_30d', 'sellers_30d', 'sales_30d', 'gmv_30d', 'video_sales_30d', 'video_gmv_30d', 'video_avg_price_30d', 'live_products_30d', 'live_sales_30d', 'live_gmv_30d', 'live_avg_price_30d', 'male_ratio', 'female_ratio', 'age_18_24', 'age_25_34', 'age_35_44', 'age_45_54', 'age_55_plus', 'main_age_group', 'main_age_ratio'];
  ...
  if (dryRun) {
    console.log('Dry run: would insert', filteredBuffer.length, 'rows');
    return;
  }
  ...
  while (retries < maxRetries) {
    try {
      ...
      retries++;
      console.error(`Insert failed (attempt ${retries}/${maxRetries}):`, error);
