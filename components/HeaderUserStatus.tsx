...
export default function HeaderUserStatus() {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-2 rounded-full border border-zinc-200 bg-white/80 px-3 py-1 text-[12px] text-zinc-700">
        <span className="font-semibold text-slate-900">本月浏览</span>
        <span className="tabular-nums text-zinc-600">{typeof limit === 'number' ? `${used ?? 0}/${limit}` : used ?? '—'}</span>
        {label ? (
          <span className="ml-1 rounded-full border border-zinc-200 bg-white px-2 py-0.5 text-[11px] font-semibold text-zinc-600">
            {label}
          </span>
        ) : null}
      </div>
